...
# Dataset
# ---------------------------------------------------------
class SARVAEDataset(Dataset):
    def __init__(self, folder_path, transform=True):
        self.folder_path = folder_path
        # Recursively collect .tif files from folder_path (including subdirectories)
        self.files = []
        for root, _, files in os.walk(folder_path):
            for fname in files:
                if fname.lower().endswith(".tif"):
                    full_path = os.path.join(root, fname)
                    # store path relative to folder_path so os.path.join(self.folder_path, ...) still works
        ...
        # self.folder_path = folder_path
        # # Only collect .tif files directly under folder_path (no subdirectories)
        # self.files = [
        #     fname for fname in os.listdir(folder_path)
        #     if fname.lower().endswith(".tif") and os.path.isfile(os.path.join(folder_path, fname))
        # ]
        # self.files = sorted(self.files)
        # print(f"Found {len(self.files)} .tif files in {folder_path}")
        # self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(img_path) as src:
            s1 = src.read().astype(np.float32)  # (13, H, W)

        # Normalize per-band
        if self.transform:
            VV_dB, VH_dB = s1[0], s1[1]
            VV_lin = 10 ** (VV_dB / 10.0)
            VH_lin = 10 ** (VH_dB / 10.0)
            ratio = VV_dB / (VH_dB)
            s1 = np.stack([VV_lin, VH_lin, ratio], axis=0)
            mean = s1.mean(axis=(1, 2), keepdims=True)
            std = s1.std(axis=(1, 2), keepdims=True) + 1e-6
            s1 = (s1 - mean) / std

        return torch.from_numpy(s1)
...
def train():
    ...
    dataset = SARVAEDataset(folder_path="/kaggle/input/sar-cr-sample")
